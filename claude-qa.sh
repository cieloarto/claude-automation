...
init_qa_team() {
    ...
    cat > /tmp/qa_init_prompt.txt << 'QAEOF'
あなたはQA & テストチームリーダーです。

【役割】
- テスト要件定義 (docs/tests/test-specifications.md)
- 品質ゲートチェック (PR作成前の必須チェック)
- 自動PR作成・レビュー承認
...
QAEOF
    ...
    # ファイルから読み込んで送信
    tmux send-keys -t "$QA_PANE" "cat /tmp/qa_init_prompt.txt" C-m
    sleep 0.5
    rm -f /tmp/qa_init_prompt.txt
}
